...
Outputs (under `results/runs/model_benchmark/<run_name>/`):
...
python scripts/benchmark/compare_models.py \
  ...
  --output_dir results/runs/model_benchmark \
  ...
  --model_a_type legacy \
  --model_a_ckpt results/checkpoints/your_legacy_best.pth \
  --model_a_legacy_cr 0.8 \
  --model_b_name nas_best \
  --model_b_type nas \
  --model_b_ckpt results/runs/nas_retrain/<run_id>/best_model.pth \
  --model_b_arch_json results/runs/nas_search/<search_run_id>/best_arch.json
```
...
python scripts/benchmark/compare_models.py \
  ...
  --model_a_type nas \
  --model_a_ckpt results/runs/nas_retrain/<top1_run>/best_model.pth \
  --model_a_arch_json results/runs/nas_search/<search_run_id>/best_arch.json \
  --model_b_name nas_top2 \
  --model_b_type nas \
  --model_b_ckpt results/runs/nas_retrain/<top2_run>/best_model.pth \
  --model_b_arch_json results/runs/nas_retrain_top3/arch_jsons/top2_<tag>.json
```
